...
class MultiHeadAttention(Module):
    ...
    def project_to_query_key_value(self, x):
        ...
        self.x_flattened = x.view(batch_size * seq_len,n_embd).contiguous()                  # (B*T, E)

        self.Xq1 = self.q_projection.forward(self.x_flattened).contiguous()
        self.Xq2 = self.Xq1.view(batch_size, seq_len,self.n_head,self.attn_hidden_dim).contiguous()
        self.Xq3 = self.Xq2.permute(0,2,1,3).contiguous()
        self.Xqflattened = self.x_flattened

        self.Xk1 = self.k_projection.forward(self.x_flattened).contiguous()
        self.Xk2 = self.Xk1.view(batch_size, seq_len,self.n_head,self.attn_hidden_dim).contiguous()
        self.Xk3 = self.Xk2.permute(0,2,1,3).contiguous()
        self.Xkflattened = self.x_flattened

        self.Xv0 = self.v_projection.forward(self.x_flattened).contiguous()
        # self.Xv0.requires_grad_(True)
        self.Xv1 = self.Xv0.view(batch_size, seq_len, self.n_embd).contiguous()
        # self.Xv1.requires_grad_(True)
        self.Xv2 = self.Xv1.view(batch_size, seq_len,self.n_head,self.attn_hidden_dim).contiguous()
        # self.Xv2.requires_grad_(True)
        self.Xv3 = self.Xv2.permute(0,2,1,3).contiguous()
        self.Xvflattened = self.x_flattened

        self.kT = self.Xk3.permute(0,1,3,2).contiguous()

        ### END YOUR SOLUTION
        return self.Xq3, self.kT, self.Xv3
    ...
    #     # k = self.reshape_and_multiply_layer(self.k_projection, x)
    #     # v = self.reshape_and_multiply_layer(self.v_projection, x)
    #     self.x_flattened = x.view(batch_size * seq_len,n_embd)                  # (B*T, E)

    #     self.Xq1 = self.q_projection(self.x_flattened)
    #     self.Xq2 = self.Xq1.view(batch_size, seq_len,self.n_head,self.attn_hidden_dim)
    #     self.Xq3 = self.Xq2.permute(0,2,1,3).contiguous()
    #     self.Xqflattened = self.x_flattened

    #     self.Xk1 = self.k_projection(self.x_flattened)
    #     self.Xk2 = self.Xk1.view(batch_size, seq_len,self.n_head,self.attn_hidden_dim)
    #     self.Xk3 = self.Xk2.permute(0,2,1,3).contiguous()
    #     self.Xkflattened = self.x_flattened

    #     self.Xv1 = self.v_projection(self.x_flattened)
    #     # self.Xv1.requires_grad_(True)
    #     self.Xv2 = self.Xv1.view(batch_size, seq_len,self.n_head,self.attn_hidden_dim)
    #     # self.Xv2.requires_grad_(True)
    #     self.Xv3 = self.Xv2.permute(0,2,1,3).contiguous()
    #     self.Xvflattened = self.x_flattened
    ...
    def self_attention(self, q, kT, v):
        ...
        ### BEGIN YOUR SOLUTION
        self.attn_scores = q@kT/(self.attn_hidden_dim**0.5)
        self.v = v
        # self.v.requires_grad_(True)
        if self.causal:
            self.attn_scores = self.attn_scores+mask

        self.attn_weights = softmax(self.attn_scores,dim=3)
        # self.attn_weights.requires_grad_(True)
        self.attn_output = self.attn_weights @ self.v
        # self.attn_output.requires_grad_(True)
        # result shape b x num_heads x seq_len x attn_hidden_dim
        ### END YOUR SOLUTION
        self.permute_output = self.attn_output.permute(0,2,1,3).contiguous()
        self.output = self.permute_output.view(batch_size, queries_len, self.attn_hidden_dim*num_head)
        return self.output
    ...
    def forward(self, x):
        ...
        res = self.out_projection.forward(res.view(batch_size*seq_len,n_embd)).view(batch_size, seq_len, n_embd)
        return res
        ### END YOUR SOLUTION
